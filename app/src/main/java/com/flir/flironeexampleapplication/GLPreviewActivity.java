...
*/
    ...
    public void onFrameProcessed(final RenderedImage renderedImage){
        ...
        if (renderedImage.imageType() == RenderedImage.ImageType.ThermalRadiometricKelvinImage){
            ...
            final String spotMeterValue = numberFormat.format(averageC) + "ºC" +testTemp;
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
            ...
            testTemp++;
            if(testTemp%100 == 0 || testTemp>20) {

                try {
                    String strThermalPixels[] = new String[thermalPixels.length];

                    for (int i = 0; i < thermalPixels.length; i++)
                        strThermalPixels[i] = numberFormat.format(((thermalPixels[i] / 100) - 273.15));

                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd-HH-mm-ssZ", Locale.getDefault());
                    String formattedDate = sdf.format(new Date());
                    String fileName = "FLIRTempData-" + formattedDate + ".csv";

                    saveToSD(fileName, Arrays.toString(strThermalPixels));
